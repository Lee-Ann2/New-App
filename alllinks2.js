...
async function fetchShows(page) {
  const response = await fetch(`https://api.themoviedb.org/3/discover/tv?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${page}&sort_by=popularity.desc&api_key=${apiKey}`);
  const data = await response.json();
  return data;
}


async function fetchSortedShows(sortBy) {
  const response = await fetch(`https://api.themoviedb.org/3/discover/tv?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${currentPage}&sort_by=${sortBy}&api_key=${apiKey}`);
  const data = await response.json();
  return data.results;
}

// Function to render movies
function renderShows(tvs) {
  movieContainer.innerHTML = '';
  tvs.forEach((tv) => {
    const movieHtml = `
      <div class="box">
        <div class="movie-info">
          <img src="https://image.tmdb.org/t/p/w400${tv.poster_path}" alt="${tv.original_name}" id="image">
          <span id="heart">
          ...
          <h3 id="title">${tv.original_name}</h3>
          <p id="rating">Rating: ${tv.vote_average}<svg width="16" height="16" viewBox="0 0 24 24" fill="#FFFF00" xmlns="http://www.w3.org/2000/svg">
          ...
          <p id="year">Release Date: ${tv.first_air_date}</p>
...
function activePage(pageNumber) {
  ...
  fetchShows(currentPage).then((data) => renderShows(data.results));
}
...
sortBtn.addEventListener('click', () => {
  ...
  fetchSortedShows(sortBy)
    .then((tvs) => renderShows(tvs))
