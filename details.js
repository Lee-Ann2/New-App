...
const options = {
  ...
  function renderAdditionalData(movie) {
    console.log('Rendering additional data:', movie);
    ...
    fetch(`https://api.themoviedb.org/3/movie/${movie.id}?api_key=${apiKey}&append_to_response=videos,credits,reviews`)
      .then(response => response.json())
      .then(data => {
        // Render trailer
        trailerContainer.innerHTML = `
          <div class="name"><h1>${data.original_title}</h1></div>
...
<div class="review">
 ...
 <ul>
   ${data.reviews.results.map((review) => `
     <li>
       <h3>${review.author}</h3>
       <p>${review.content}<span id="dots"></span><span id="more"></span><button id="myBtn" onclick="myFunction()">see more</button></p>
