...
form.addEventListener('submit', e => {
    e.preventDefault();
    validateInputs(); // validating all the inputs so we call the function
});

const setError = (element, message) => {
    // since we have input fields we need to set errors
...
const validateInputs = () => {
    const nameValue = name.value.trim(); // trim to eliminate the white spaces
    ...
    if(authPasswordValue !== passwordsValue || authPasswordValue === '') {
        setError(authPassword, 'Invalid password or the passwords are not the same');
    } else {
